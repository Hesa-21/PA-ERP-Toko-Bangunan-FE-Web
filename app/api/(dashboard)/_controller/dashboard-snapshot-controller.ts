...
import { buildDashboardSnapshot } from "@/app/api/(dashboard)/_service/dashboard-service"

export async function handleDashboardSnapshotGet(request: Request) {
  ...
  try {
    const dto = buildDashboardSnapshot({
      branch: query.branch,
      scope: query.scope,
      limit: query.limit,
    })
    ...
    return mapDashboardRouteError(err, "Failed to load dashboard snapshot")
  }
}
